...
2026-09-17. 직접 실행하는 웹 서버의 시작·Ctrl+C 종료·같은 포트 재시작, 포트 충돌·시작 실패 안내, 같은 출력 폴더의 동시 생성과 패키지 보존, 오래된 pid 기록을 무시하는 동작을 세 운영체제의 공통 시험으로 옮기거나 추가했다. 웹 시험은 22개에서 25개로 늘고, 윈도의 POSIX 전용 건너뛰기는 5개에서 3개로 줄었다. 옮긴 두 포트 오류 시험은 POSIX에서 기존 백그라운드 제어 명령의 단언도 실행한다. 백그라운드 명령, 잠금으로 pid의 주인을 판별하는 동작, 셸·Finder 실행 파일의 세 시험은 POSIX에 남겼다.

이 과정에서 윈도는 `HTTPServer`가 상속한 `SO_REUSEADDR` 때문에 두 서버가 같은 포트를 함께 열 수 있음을 재현했다. `web/server.py`에서 윈도에 한해 주소 재사용을 끄고, 먼저 켠 서버는 계속 응답하면서 두 번째 서버가 포트 오류로 끝나는지 확인한다. POSIX의 주소 재사용과 역방향 이름 조회를 피하는 기존 동작은 유지한다. 생성 패키지에 들어가는 소스와 의존성은 바꾸지 않았다.

시험 도우미 `tests/web_process.py`는 윈도 서버마다 숨겨진 콘솔을 만든다. 비대화형 셸의 Ctrl+C 무시 설정이 자식에게 상속될 수 있으므로, 전용 실행 도우미가 그 설정을 해제한 뒤 실제 `python -m hanok_generator.web` 명령을 켠다. 실행 도우미 자신은 Ctrl+C를 무시하고 응용 프로그램의 종료 코드를 전달한다. 별도 도우미가 이 전용 콘솔에만 Ctrl+C를 보내며 시험 실행기의 콘솔은 건드리지 않는다.

서버의 정상 종료 문구와 종료 코드 0을 단언하며, 시간 초과 때 프로세스 트리를 강제로 정리해도 시험 실패는 그대로 남는다. 생성이 진행 중인 상태에서 Ctrl+C를 보내는 시험은 아니다. 죽은 자식의 pid와 살아 있는 별도 감시 프로세스의 pid를 기록한 두 경우에 직접 실행한 서버가 그 기록을 그대로 두고 감시 프로세스도 살아 있는지 확인한다. 이는 윈도에 POSIX 잠금을 이식한 것이 아니라 직접 실행 경로가 제어 기록을 사용하지 않는다는 검증이다.

### 도구 설명의 검사 수 계산 (#31)

2026-09-17. `engine/validation_rules.py`에 완성 패키지 검사의 ID를 실행 순서대로 기록하고, 서버 메타데이터의 `validation_check_count`를 이 목록의 길이로 계산한다. `build_package`의 네 형식 설명과 `describe_generator`의 생성물 안내가 그 값을 사용하며, 생성 진행 안내에서는 고정 숫자를 뺐다. 패키지가 없어도 조회할 수 있고 웹·도구 프로세스는 계속 CAD 빌더와 `ezdxf`를 불러오지 않는다. 기존 패키지의 검사 수는 그 패키지의 검증 기록에서 읽는다.

빌더의 계산과 검사 호출은 바꾸지 않았다. 목록에는 부품 계열별·이음별 반복 검사를 펼쳐 적었고, 명세 파일이 있는 완성 패키지의 검사만을 기준으로 삼는다. 생성기 시험이 단문 양쪽 경첩·창살 0개·R3·액자형을 포함한 모든 시험 설계에서 실제 검사 ID·순서·개수와 중복 여부를 대조한다. 새 검사를 넣으면 이 목록도 고쳐야 하지만 도구 설명은 자동으로 따라간다. 도구 시험은 목록에 임시 규칙을 추가했을 때 네 형식 설명과 생성물 안내가 함께 바뀌고, 이미 생성한 패키지의 검사 수는 그대로인지 확인한다. 진행 단계와 프로세스 격리 시험도 유지한다.

새 목록 파일은 패키지의 `source/`에 함께 들어가므로 패키지 파일은 36개에서 37개로 늘고 모든 입력의 패키지 ID가 바뀐다. 매니페스트까지 포함한 ZIP은 38개다. 실제 검사는 71개 그대로이며, 도면·검증 기록의 동작은 바꾸지 않았다. `tests/results.json`은 생성기 시험 스크립트를 실행해 새 소스 해시와 결과를 기록한다. 기존 예제 패키지와 화면 이미지는 생성 당시 기록으로 남기고 설명서에 파일 개수 차이를 적었다.

### 도구 결과 스키마의 표준 검증 (#32)

2026-09-17. `tests/test_llm.py`의 일부 키워드만 확인하던 `conforms()`를 없애고 시험 전용 `jsonschema`의 `Draft202012Validator`를 사용한다. MCP 도구 정의에 공개한 스키마의 문법과 여덟 도구의 성공·오류 결과, 생성 실패 결과를 검사한다. 실제 결과의 속성 타입과 필수 상태를 일부러 바꾸어 스키마 제약이 사라져도 시험이 통과하는 일을 막는다.

같은 검증 함수를 사용하는 별도 예제로 추가 속성 금지, 열거 값, 숫자 범위와 경계, 상수, 분기 일치 수, 중첩 객체·배열, 필수 속성, 숫자 자리에 들어온 불리언을 확인한다. 정상 값과 선택·널 허용 속성도 함께 확인한다. 실제 출력 스키마의 추가 속성 허용과 모델 API별 호환성은 그대로다. 빈 스키마와 확장 키워드는 표준상 유효하므로 문법 검사만으로 모든 계약 약화를 검출한다고 주장하지 않는다.

`requirements-test.lock`은 기존 실행 잠금 파일을 참조하고 시험 의존성만 추가한다. 세 운영체제 CI는 이 파일로 설치하며 pip 캐시 키에 두 잠금 파일을 모두 반영한다. 일반 설치의 의존성, 소스 묶음과 패키지 생성 코드는 바꾸지 않는다. 시험 설치 절차는 `README.md`에 따로 적었다.

### 설명서 그림의 윈도 실행과 최신 여부 확인 (#33)

2026-09-17. 설명서 촬영 스크립트가 윈도의 시스템·32비트·사용자 Chrome 설치 폴더와 `PATH`를 찾는다. `--chrome`은 공백을 포함한 절대·상대 경로를 받으며 지정한 실행 파일이 없으면 오류를 낸다. 기존 macOS 앱과 Linux 명령 탐색, DevTools 포트와 임시 브라우저 프로필을 유지한다.

`--check`는 그림 20장을 임시 폴더에 다시 만들고 기준 폴더의 크기와 RGBA 픽셀을 비교한다. PNG 부가 정보·압축 차이는 무시하며 같음은 0, 크기·픽셀 차이와 누락·추가 PNG는 1, 실행·읽기 오류는 2로 끝난다. `--out`은 일반 실행에서는 저장 폴더, 검사에서는 읽기 전용 기준 폴더다. 모든 촬영이 끝난 뒤에만 기준 그림을 갱신하므로 촬영 실패나 검사 과정은 기존 그림을 바꾸지 않는다.

임시 예제 폴더의 생성 시각, 화면의 실패 시간·작업 기록 이름·출력 경로를 설명용 값으로 고정했다. 부분 그림을 찍을 때 상단 메뉴가 내용을 가리지 않게 고정을 잠시 풀고 되돌린다. 정상 실행과 실패, 초기화 도중 오류에서도 이 실행이 만든 브라우저·소켓·서버·임시 프로필을 정리한다. 생성기와 실제 웹 화면 코드는 바꾸지 않았다.

그림 20장을 Windows 11·Chrome 151에서 다시 찍고 검사 71개, 파일 37개와 ZIP 38개를 표시하는 현재 화면에 설명을 맞췄다. 촬영 환경과 세 운영체제의 생성·검사 명령은 설명서에 적었다. 같은 환경의 반복 촬영은 20장 모두 같았고, 임시 기준 사본의 픽셀 하나를 바꾸거나 그림 하나를 지우면 각각 해당 파일을 지목하며 1로 끝났다. 실제 Chrome·서버를 띄운 뒤 촬영 실패를 주입해 종료 코드 2, 프로세스 종료·프로필 삭제·소켓과 포트 닫힘, 기준 그림과 실제 `output/` 보존을 확인했다.

새 시험은 브라우저 없이 경로 탐색, 픽셀·크기·알파·부가 정보 비교, 파일 보존과 초기화·종료 오류를 확인한다. 세 운영체제 CI에 이 시험을 더했으며 실제 그림 최신 여부 검사는 필요할 때 수동으로 실행한다. 다른 운영체제·글꼴·브라우저·의존성·ODA 설치 상태 사이의 픽셀 일치는 보장하지 않는다. 실행 의존성과 패키지 소스 경계는 그대로다.

### 결정적 출력 형식의 등록 지점 (#34)

2026-09-18. 후보 2만 구현했다. `engine/output_formats.py`에 AI의 파일 이름·쓰기·읽기 검증·검사 ID·보고 위치를 등록하고, 빌더가 후보 쓰기·두 단계 검증·정리·교체를 순회한다. 같은 등록 정보가 필수 파일, 검사 목록의 기존 AI 위치, 생성 README의 파일 이름에도 연결된다. 새 형식은 구현 모듈과 등록 모듈을 고치면 이 연결 위치들을 재사용한다. PNG·CSV·README 함수의 분리와 전체 검사 이름 상수화는 후속 범위다. [등록 안내](OUTPUT_FORMATS.md)에 확장 절차와 호환 범위를 적었다.

등록 메타데이터만 읽을 때 CAD 모듈을 불러오지 않는다. 기존 `Design.AI`와 `ai_path` 위치·키워드 인자, 명세 파일 없는 독립 검증을 유지한다. 새로운 형식의 검증 경로는 공통 `export_paths`로 전달한다. 쓰기·검증·저장 후 읽기 오류에서 모든 후보를 정리하며, 공개 생성 경로의 기존 패키지·최신 포인터 보존을 확인했다. 직접 빌더를 부를 때 여러 파일 교체 전체가 원자적인 것은 아니다.

완료 기준의 변경 전후 패키지 ID 불변은 소스를 함께 보관하는 패키지 구조와 맞지 않아, 사용자에게 제안한 기준에 따라 생성 내용 불변·소스 차이 설명·변경 후 재현으로 대체했다. Windows 11·CPython 3.11.9·고정 의존성·동일 글꼴 환경에서 기준 `8b59388`과 변경 후 예제 5종을 새로 생성했다. 각 예제의 내용 파일 17개(DXF·AI·PNG·CSV·설계 JSON·README·검증 기록)가 바이트까지 같았고, 검사 71개의 순서·실측값·리비전도 같았다. 각 예제의 반복 생성과 포함 소스를 이용한 최적화 실행도 전체 파일·패키지 ID가 같았다.

기존 파일 중 달라진 것은 `builder.py`·`validation_rules.py`·`package.py`의 포함 소스, `environment.json`의 `source_sha256`, 매니페스트뿐이다. 새 등록 모듈의 포함 소스가 추가되어 패키지는 37개에서 38개, 매니페스트를 포함한 ZIP은 39개가 됐다. ID 계산·무결성 검사와 기존 운영체제 간 비교 예외는 유지한다. 과거 예제·그림 기록을 보존하고 현재 파일 개수와 차이를 문서에 적었다.

출력 등록 시험 10개를 세 운영체제 CI에 추가했다. 시험용 출력을 실제 작업 프로세스에 등록해 쓰기·검사 추가·필수 파일·README·봉인을 확인하고, 누락한 출력을 매니페스트에서 함께 지워도 검증이 거절하는지 확인한다. 로컬 생성기 시험 24개는 일반·최적화 소수 외곽 각 200건과 실제 ODA 27.1.0의 R3·액자형 A2 왕복을 포함해 통과했다(232.7초). `tests/results.json`은 이 실제 실행으로 다시 기록했다.

### 0.5.0 릴리스 준비 (#35)

2026-09-18. [릴리스 안내](RELEASE_0.5.0.md)에 마지막 태그 `v0.4.1` 이후의 변경, #11~#18과 #27~#34의 반영 범위, 모든 패키지 ID의 변경과 `revision` 비교 방법을 정리했다. 현재 검사 71개·파일 38개·매니페스트 포함 39개를 적고, #34만의 생성 내용 불변 결과를 0.4.x에서 0.5.0으로의 전환과 구분했다. 두 README에서 안내를 연결하고 저장소 README의 시험 명령을 시험 잠금 파일과 일곱 묶음에 맞췄다. 생성기 README 첫머리의 DXF 불변 설명도 릴리스 전체의 도면 변경에 맞게 고쳤다.

배포 버전 0.5.0, 엔진 버전 0.3.0, 실행 코드·의존성·포함 소스·ID 계산은 그대로다. 과거 예제와 설명서 그림도 당시 기록으로 보존한다. Illustrator와 대상 DWG 프로그램에서 직접 여는 수동 확인은 수행하지 않았으며, 릴리스 안내에 그 상태를 적었다.

브랜치 정리 대상도 다시 확인했다. 원격 `feat/issue-17-dwg-export`와 `feat/issue-18-ai-export`는 이미 없었다. `docs/example-packages`의 `83440a6`은 `main`에 병합되지 않은 예제 보관 커밋으로, 생성 파일 175개가 들어 있다. 이 브랜치를 병합 완료 브랜치로 간주하지 않고 보존한다. 다른 브랜치와 로컬 작업 트리는 정리하지 않는다.

이번 변경은 게시 전 문서 준비다. 준비 PR을 병합한 뒤 그 `main` 커밋의 CI가 모두 통과하면 해당 커밋에 주석 있는 `v0.5.0` 태그를 만들고 한국어 릴리스를 게시한다. 게시와 원격 태그를 확인한 뒤 #35의 상태를 갱신하며, 이 문서 준비만으로 태그·게시·수동 확인을 완료 처리하지 않는다.
