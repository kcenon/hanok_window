...
창의 외경(완성 외곽) 또는 내경(고정틀 안목) 가로·세로, 창짝당 창살 수, 단문(왼쪽·오른쪽 경첩)·양문을 입력하면 CNC용 한옥 창호 패키지를 만듭니다. 패키지에는 DXF, AI 가공 도면, PNG 5장, CSV 4종과 저장한 DXF를 다시 읽어 확인한 검증 기록이 들어 있습니다.
...
0.4.x에서 0.5.0으로 전환하면 패키지에 들어가는 소스가 달라져 모든 패키지 ID가 바뀝니다. 같은 설계인지 비교할 때는 revision을 사용합니다. 참고 도면·가공 메모의 위치와 홈 색 등을 고쳤으므로 DXF·PNG도 달라질 수 있습니다. 전환 시 달라지는 파일과 현재 검사·파일 수는 [0.5.0 릴리스 안내](docs/RELEASE_0.5.0.md)에 정리했습니다.

함께 볼 문서: [그림으로 보는 사용 설명서](docs/manual/README.md) · [생성 예제 5종](examples/README.md) · [0.5.0 릴리스 안내](docs/RELEASE_0.5.0.md) · [버전별 구현·검증 기록](docs/CHANGELOG.md) · [저장소 안내](../README.md) · [R3 원본 안내](../r3_reference/00_START_HERE.txt)
...
| 화면 | 하는 일 |
|---|---|
| 설계 `#/design` | 형식·외경/내경/화판·창살·프리셋·그림·원판을 입력합니다. 입력이 멈추고 150 ms 뒤 서버가 해석해 정면도와 원판 배치를 그리고, 규칙 위반은 해당 칸 옆에 원인과 고치는 방법을 적고, 크기·창살·그림·원판 규칙은 엔진이 받아들이는 값까지 적습니다(예: “외경 가로를 473 mm 이하로 줄이거나 외경 세로를 751 mm 이상으로 늘리세요.”). 외경·내경·화판 기준을 바꾸면 같은 창이 되도록 숫자를 바꿔 넣습니다. 화판 기준에서는 화판 칸이 나타나고 그림 칸이 잠깁니다. JSON 불러오기·저장 |
...
`build_package` 설명과 `describe_generator`가 알리는 검사 수는 서버 메타데이터의 `validation_check_count`에서 가져옵니다. 이 값은 CAD 라이브러리를 불러오지 않는 `engine/validation_rules.py`의 `RULE_IDS` 길이이며, 패키지가 없어도 조회할 수 있습니다. 검사 규칙을 추가하거나 순서를 바꾸면 이 목록도 함께 고칩니다. 생성기 시험이 모든 시험 설계의 실제 검사 기록과 ID·순서·중복 여부를 대조하므로 목록만 뒤처져도 실패합니다. 이미 만든 패키지의 통과 수와 전체 수는 그 패키지의 검증 기록에서 계속 읽습니다.
...
패키지 화면의 **DWG 받기**는 봉인된 `window.dxf`를 AutoCAD 2010 형식 DWG로 바꿔 내려 줍니다.
ODA File Converter가 설치된 PC에서만 동작하고(`winget install ODA.ODAFileConverter`),
없으면 그 줄이 설치 안내로 바뀔 뿐 나머지 기능은 그대로입니다. `HANOK_ODAFC`에 실행 파일 경로를 주면 그것을 씁니다.
