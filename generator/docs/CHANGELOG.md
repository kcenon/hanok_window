...
시험 도우미 `tests/web_process.py`는 윈도 서버마다 숨겨진 콘솔을 만들고 별도 도우미가 그 콘솔에만 Ctrl+C를 보낸다. 서버의 정상 종료 문구와 종료 코드 0을 단언하며, 시간 초과 때 프로세스 트리를 강제로 정리해도 시험 실패는 그대로 남는다. 생성이 진행 중인 상태에서 Ctrl+C를 보내는 시험은 아니다. 죽은 자식의 pid와 살아 있는 별도 감시 프로세스의 pid를 기록한 두 경우에 직접 실행한 서버가 그 기록을 그대로 두고 감시 프로세스도 살아 있는지 확인한다. 이는 윈도에 POSIX 잠금을 이식한 것이 아니라 직접 실행 경로가 제어 기록을 사용하지 않는다는 검증이다.
