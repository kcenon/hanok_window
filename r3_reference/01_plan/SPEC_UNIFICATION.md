# 규격 통일·검증 보강 기록 / PORTRAIT_DL_R3

작성일 2026-09-11. 이 문서는 `hanok_window/` 아래 두 패키지를 하나로 합친 근거와 변경 내역입니다.
1~5절은 R2 통합 당시의 기록이며, 현재 R3의 검증 보강은 7절에 기록합니다.
...
6. 재료·가공 조건별 최소 홈 사이 폭, 비개방 가장자리 폭, 잔존 두께
...
## 7. R3: 수치 비교와 실제 절삭 영역 검증

R2 패키지 23개 파일과 매니페스트를 수정 전에 `research/baselines/PORTRAIT_DL_R2.tar.gz`로
보관했습니다. 아카이브 SHA-256은 `ba2a2c567878ae0de9d8e0fa4b597b197d6a777975baa4ed3469bdac0a62b3db`입니다.
임시 복사본의 R2 재빌드에서 `design_spec.json`과 DXF가 기존 바이트와 일치했습니다.
R3는 기본 설계의 부품·홈·도그본·결합 ID와 원판 배치를 그대로 유지합니다.
DXF에는 R3 리비전과 수치 정책 메타데이터가 기록되므로 R2와 파일 해시는 다릅니다.

| 수정 | 구현 |
|---|---|
| 최적화 옵션에서 사라지는 입력 단언 | 유효 숫자·치수 관계·세장비·깊이·공구 조건을 명시적 `ParameterError`로 변경 |
| 소수 치수의 관계식·그림·고정틀 비교 실패 | `numeric_policy.py`의 단위별 비교 적용. 좌표를 반올림해 집합으로 비교하던 곳도 선형 오차로 대조 |
| 그림의 최소 여유만 확인 | `picture_margins_match_each_side`: 실제 좌·우·하·상 네 값 확인 |
| 공구보다 작은 도그본 통과 | `machining_relief_radius_at_least_tool_radius`: metadata 반경 대신 실제 원호 반경 확인 |
| 과밀 창살의 절삭 영역 연결 | `distinct_machining_regions_separated`: 홈과 자신의 도그본을 합쳐 같은 부재·겹치는 깊이의 모든 쌍 비교 |
| 도그본이 비개방 가장자리까지 닿는 경우 | `machining_preserves_non_open_edges`: 의도한 개방 경계와 보존할 변 구분 |
| 불필요한 삽입 길이/개구부 상한 | 겹침·접촉 검사와 함께 제거. 막힌 안착 홈의 `0 < lap < member_width`는 유지 |
| 원판 한 방향 초과·소수 폭 선반 분리 | 배치 전 X/Y 적재 가능성 확인, 선반 계열은 의도한 부재 폭으로 식별 |
| 수치 정책 추적 | `numeric_policy_recorded`: DXF와 리포트에 `HANOK_NUMERIC_V1` 기록 |

자동 검사는 59개에서 64개가 되었습니다. 제조용 최소 폭·두께는 임의로 정하지 않았습니다.
기본 설계의 실측 최소 홈 사이 폭은 약 29.266667mm, 비개방 가장자리 폭은 16.8mm이며,
필요 하한은 `null`, 제작 평가는 `PENDING`입니다.

원호 오차를 보장하려면 `make_path(...).flattening(distance=1e-5)`만으로 충분하지 않았습니다.
이 경로는 원호를 베지어 곡선으로 먼저 근사합니다. R2의 R3.2 도그본에서 실제 원에 대한
꼭짓점 반경 오차가 약 0.000872091mm로 측정됐습니다. 따라서 이전 기록의 1e-5mm는
원 자체에 대한 총 오차 한계로 해석하면 안 됩니다. R3는 원호의 실제 중심·반경·bulge에서
직접 현을 계산합니다. 시험한 R3.2 원의 최대 현 중앙 오차는 약 0.000009884mm이며,
접촉 판정에는 양쪽 현 근사 오차와 좌표 오차를 합쳐 사용합니다.
[ezdxf Path의 원호 근사 설명](https://ezdxf.readthedocs.io/en/stable/path.html)

회귀 시험은 `03_tests/test_regressions.py`로 재실행합니다. 200개 소수 외곽 치수는 각각
DXF를 생성·저장·재읽기하며, 그림·공구 후단 검사는 입력 검증을 우회한 변조 파일에서도
실패해야 합니다. 과밀 12+4, 삽입 상한 제거 대조군, 원호 접촉, 비개방 가장자리 접촉도
포함합니다. 간극 3.1/부재 폭 30.2와 여러 부재·간극·두께·공구 소수를 함께 바꾼 정상
사례도 검사합니다. 일반 및 `PYTHONOPTIMIZE=1` 실행의 실제 결과와 소스 해시는
`03_tests/regression_results.json`에 기록합니다. 전체 입력 공간이나 실제 제작 강도의
증명으로 확대해서 해석하지 않습니다.

### 7.1 교차검증 후 보완

R3 적용 뒤 교차검증에서 네 가지가 확인되어 R3에 반영했습니다. 기본 설계의 형상은 바뀌지
않았고 DXF 해시도 `34bf267a…` 그대로입니다.

| 발견 | 재현 | 조치 |
|---|---|---|
| 회귀 기록의 일반·-O DXF 해시 일치가 우연 | 시험이 `build()`를 직접 호출해 시드 0 재실행을 거치지 않음. 시드 0~7로 기본 설계를 빌드하면 CLASSES 섹션의 `LAYOUT`/`ACDBPLACEHOLDER` 순서가 4:4로 갈림(8줄, 형상 무관). 시드를 고정하지 않은 다른 실행에서는 일반·-O가 210건 중 0건 일치 | 시험도 `PYTHONHASHSEED=0`으로 재실행. `--record`가 두 실행을 묶고 해시 일치·계획서 대응·R2 윤곽 비교를 코드로 계산해 기록 |
| 도그본이 닫힌 외측 변을 뚫는 입력이 파라미터 단계를 통과 | 테두리 26.6·삽입 24.0·R2.72 등. 저장 도형 검사에서만 거부됨 | `lattice.seat_relief_inside_member`: `테두리 폭 − 삽입 길이 − 도그본 반경 > 1e-9 mm`. 저장 도형의 가장자리 검사는 유지. 2 µm 차이처럼 이 규칙은 통과하지만 원호·좌표 오차 범위 안인 경우는 저장 도형 검사가 거부함을 회귀 시험에 포함 |
| 지원하지 않는 단문·창살 0개를 입력 단계에서 허용 | 창짝 1은 하드웨어 검사, 창살 0개는 간격 검사에서 항상 실패 | `scope.leaf_count`(창짝 2만), `scope.lattice_count`(방향별 1개 이상) |
| 검증에 실패한 빌드가 `design_spec.json`만 갱신 | 규격을 검증 전에 기록해, 실패하면 새 규격과 이전 DXF가 섞여 남음(R2부터) | 규격·DXF를 후보 파일로 만들어 저장 전·후 검증을 모두 통과한 뒤 교체. 실패 시 후보 삭제. `test_failed_build_leaves_outputs_untouched` |

모든 길이 값을 0.1 mm 단위 소수로 바꾸고 창짝 수·창살 수를 섞은 무작위 설계 300개를
실제 빌더로 저장·재읽기했습니다(교차검증용 시험, 패키지 밖).

| 결과 | 수정 전 | 수정 후 |
|---|---|---|
| 64개 통과 | 137 | 137 |
| 저장 도형 검증 실패 | 163 (단문 83, 양문 창살 0개 79, 도그본 1) | 0 |
| 파라미터 단계 거부 | 0 | 163 (`scope.leaf_count` 83, `scope.lattice_count` 79, `lattice.seat_relief_inside_member` 1) |

소수 입력 때문에 생긴 실패는 수정 전후 모두 0건입니다. 통과한 설계에는 비개방 가장자리
폭이 0.13 mm인 사례도 있습니다. 명목 기하 합격과 제작용 최소 폭(PENDING)이 별개임을
보여 줍니다.

패키지 복사본에서 창살을 과밀 12+4로 바꿔 명령줄 빌드를 실패시키면, 수정 전에는
`design_spec.json`이 새 값으로 바뀌어 이전 DXF와 섞였고, 수정 후에는 `design_parameters.json`
외에 바뀐 파일과 남은 후보 파일이 없습니다.

`--record` 결과: 두 실행 모두 시험 8개 PASS, DXF를 만든 211건의 해시 211건 일치,
64개 검사 모두 `MERGED_PLAN.md`에 대응, R2 대비 생산 윤곽 176/176 동일(형상·메타데이터).
`PYTHONHASHSEED`를 지정하지 않고 일반·`PYTHONOPTIMIZE=1`로 각각 실행한 소수 200건도
시험의 자체 재실행 덕분에 해시 200건이 모두 일치했습니다(수정 전 같은 조건 0/210).
