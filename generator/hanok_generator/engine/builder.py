...
def configure(parameters, output):
    ...
    AI=OUT/output_formats.AI.filename
    PARAMS=parameters
    D=derive(PARAMS)
...
def build(cfg):
    ...
    candidates={fmt.filename:cfg.OUT/('_validated_candidate_'+fmt.filename) for fmt in output_formats.FORMATS}
    try:
        ...
        for fmt in output_formats.FORMATS:fmt.write(cfg,doc,candidates[fmt.filename])
        pre=validate(cfg,doc,'IN_MEMORY_BEFORE_SAVE',tmpspec,export_paths=candidates)
        with open(tmp,'wt',encoding=doc.output_encoding,errors='dxfreplace',newline='\n') as fp:doc.write(fp)
        checkdoc=ezdxf.readfile(tmp)
        report=validate(cfg,checkdoc,'READ_BACK_FROM_SAVED_DXF',tmpspec,export_paths=candidates)
        tmpspec.replace(cfg.SPEC);tmp.replace(cfg.DXF)
        for fmt in output_formats.FORMATS:candidates[fmt.filename].replace(fmt.path(cfg))
    finally:
        for candidate in (tmp,tmpspec,*candidates.values()):candidate.unlink(missing_ok=True)
...
def validate(cfg,doc,phase,spec_path=None,ai_path=None,*,export_paths=None):
    ...
    # Keep the old positional/keyword AI override; new formats use the shared mapping.
    paths=dict(export_paths or {})
    if ai_path is not None:paths[output_formats.AI.filename]=ai_path
    check_exports(cfg,check,doc,paths)
...
def check_exports(cfg,check,doc,paths):
    """Read each registered export and measure it against the current DXF document."""
    for fmt in output_formats.FORMATS:
        ok,measured=fmt.verify(doc,paths.get(fmt.filename,fmt.path(cfg)))
        check(fmt.rule_id,ok,measured,target=fmt.target)
