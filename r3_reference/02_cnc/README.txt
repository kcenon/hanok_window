A3 세로형 한식 양개 창호 — 통합 CNC 패키지 / PORTRAIT_DL_R3
================================================================
...
실제 저장 DXF 재읽기: PASS_NOMINAL_DXF_GEOMETRY / 64개 검사 PASS.
...
9. 검증과 한계
실제로 생성한 DXF를 저장한 뒤 ezdxf로 다시 읽어 64개 검사 PASS.
...
곡선포함 재료검사는 실제 원호의 현 오차를 1e-05 mm 이하로 제한한 다각형 계산이며
반경/원호/bulge/내부포함은 별도로 DXF 원데이터에서도 확인합니다.
수치 정책 HANOK_NUMERIC_V1: 치수 관계식 오차 1e-9 mm, 경계·좌표 편차 1e-07 mm.
면적 차만으로 도형 동등성을 판정하지 않고 양쪽 경계 전체의 거리도 확인합니다.
실제 그림의 좌·우·하·상 여유를 각각 10 mm와 비교합니다.
...
현재 실측 최소 홈 사이 폭: 29.266667 mm.
현재 실측 최소 비개방 가장자리 폭: 16.800000 mm.
