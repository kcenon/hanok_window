...
def load_spec():
    ...
    s=build_spec(PARAMS)
    SPEC.write_text(json.dumps(s,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
...
def validate(doc,phase):
    ...
    check(f'board_boundary_{BL:g}x{BWD:g}',len(boardents)==1 and entity_polygon(boardents[0]).equals(box(0,0,BL,BWD)))
    check('all_parts_within_board',all(box(0,0,BL,BWD).covers(g) for g in partgeo.values()))
    ...
    for e in seats:
        ...
        u,v,w,h=d['local_rect'];wantcenters={(round(u,7),round(v,7)),(round(u+w,7),round(v,7))}
        gotcenters=set()
        for de in items:
            ...
            dogok &= abs(radius-R)<TOL and dd['part_id']==d['part_id'] and dd['depth_mm']==DEPTH
    ...
    check('layer_depth_and_face_separation',all(meta(e).get('depth_mm')==THK for e in rawparts) and all(meta(e).get('depth_mm')==DEPTH and meta(e).get('machining_face')=='A' for e in pockets+dogs))
    ...
    for pid,g in ap.items():
        ...
        solids[pid]=[(r,z0,z1) for r,z0,z1 in pieces if not r.is_empty and r.area>TOL and z1>z0]
    v_max=0.;worst=None
    for a,b in pairs:
    ...
    check('no_nominal_assembled_solid_interpenetration',v_max<1e-5,
          {'max_volume_mm3':v_max,'worst_pair':worst,
           'method':f'per-region Z ranges from depth {DEPTH:g} in stock {THK:g}; '
                    'arcs flattened with <=0.00001 mm tolerance'})
    ...
    check(f'frame_{W:g}x{H:g}_inner{D["inner_w"]:g}x{D["inner_h"]:g}',fixed.equals(box(0,0,W,H).difference(box(FW,FW,W-FW,H-FW))))
    check(f'all_leaf_envelopes{LW:g}x{LH:g}',all(
        tuple(round(c,7) for c in lf.bounds)==tuple(round(c,7) for c in LEAVES[i]) for i,lf in enumerate(leaves)))
    ...
    check(f'leaf_height_to_width_ratio_at_least_{MINR:g}',all(r>=MINR-TOL for r in ratios),
    ...
    check('no_fixed_centre_mullion',fixed.intersection(box(FW,FW,W-FW,H-FW)).area<TOL)
    ...
    check('assembly_reference_matches_all_cut_parts',len(abodies)==NPART and all(translate(entity_polygon(e),-ox,-oy).symmetric_difference(ap[meta(e)['part_id']]).area<1e-6 for e in abodies))
    ...
    check(f'A3_portrait{A3W:g}x{A3H:g}_in{PRW:g}x{PRH:g}_margin{PMG:g}',
          pg.equals(box(PX+PMG,PY+PMG,PX+PMG+A3W,PY+PMG+A3H)) and rg.equals(box(PX,PY,PX+PRW,PY+PRH))
    ...
    check(f'{NLEAF}_openings_{OW:g}x{OH:g}_reference',len(openings)==NLEAF and all(abs(entity_polygon(e).area-OW*OH)<TOL for e in openings))
    ...
    if SPEC.is_file():
        ...
        check('design_spec_on_disk_matches_parameters',fresh==json.dumps(json.loads(SPEC.read_text(encoding='utf-8')),sort_keys=True))
    audit=doc.audit()
...
'''
    ...
    (OUT/'requirements.txt').write_text(
        ...
        f'# Newer pins need Python 3.10+; these are the last releases that also run on 3.9.\n'
...
def reexec_with_fixed_hash_seed():
    ...
    ezdxf walks a few string-keyed collections while writing the OBJECTS section,
    and Python randomises string hashing per process, so two runs otherwise order
    a LAYOUT and its placeholder differently. Everything else is already fixed, so
