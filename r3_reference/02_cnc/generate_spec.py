...
def derive(params: dict) -> Derived:
    ...
    positive = {
        ...
        'leaf': ('member_width','min_height_to_width_ratio'),
        'lattice': ('bar_width','end_lap_length'),
        'stock': ('length','width','thickness'),
        'machining': ('tool_diameter','pocket_depth','relief_radius'),
        'picture': ('sheet_width','sheet_height'),
    }
    nonnegative = {'clearance': ('frame_to_leaf','leaf_to_leaf'),
    ...
    # The geometry below is general, but this package assigns hardware and draws
    # openings only for two outer-hinged leaves, and its spacing and detail rules
    # assume bars in both directions. Refuse the rest here instead of letting them
    # fail late in DXF validation.
    require(leaf['count']==2, 'scope.leaf_count', actual=leaf['count'], supported=[2],
            reason='single-leaf hinge side, hardware and opening drawing are not implemented')
    require(lat['vertical_per_leaf']>=1 and lat['horizontal_per_leaf']>=1, 'scope.lattice_count',
            vertical=lat['vertical_per_leaf'], horizontal=lat['horizontal_per_leaf'],
            reason='an empty lattice direction has no spacing or detail rules yet')
    ...
    require(close(region_w, pic['sheet_width']+2*pic['region_margin']),
            'picture.region_width', region=region_w,
            required=pic['sheet_width']+2*pic['region_margin'])
    require(close(region_h, pic['sheet_height']+2*pic['region_margin']),
            'picture.region_height', region=region_h,
